<?php
	...
	// Parágrafo Primeiro (cada linha em um bloco de codigo)
	$pdf->SetFont('Helvetica','B',8);
	$pdf->Cell(22, 4, 'Parágrafo Primeiro:', 0, 0, 'L');
	$pdf->SetFont('Helvetica','',8);
	$pdf->Cell(3, 4, 'Uma vez que o', 0, 0, 'L');
	$pdf->SetFont('Helvetica','B',8);
	$pdf->Cell(23, 4, 'CONTRATANTE,', 0, 0, 'L');		
	$pdf->SetFont('Helvetica','',8);
	$pdf->Cell(0, 4, 'aceite um trabalho oferecido pela', 0, 1, 'L');	
	$pdf->Cell(23, 4, 'CONTRATADA,', 0, 0, 'L');		
	$pdf->SetFont('Helvetica','',8);
	$pdf->Cell(0, 4, ', ele deverá comparecer ao local informado,', 0, 1, 'L');
	
	$pdf->Cell(0, 4, 'na data e hora comunicadas, sob pena de multa de 10% (dez por cento) do seu cachê líquido em caso de atraso, ou, na hipótese de não comparecimento, ', 0, 1, 'L');

	$pdf->Cell(3, 4, 'o', 0, 0, 'L');
	$pdf->SetFont('Helvetica','B',8);
	$pdf->Cell(23, 4, 'CONTRATANTE,', 0, 0, 'L');		
	$pdf->SetFont('Helvetica','',8);
	$pdf->Cell(0, 4, 'poderá sofrer as sanções previstas neste Contrato.', 0, 1, 'L');	
	$pdf->Ln(4);
