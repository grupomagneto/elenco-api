<?
	...
	array_push($colunas, "uf", "cep", "ator", "fashion", "drt", "drt_tipo", "exclusivo", "cd_qualificacao", "outra_agencia", "outra_agencia_nome", "eventos", "figuracao");
	...
	array_push($valores, toString($uf), toString($cep), $ator, $fashion, $drt, toString($drt_tipo), $exclusivo, $cd_qualificacao, $outra_agencia, toString($outra_agencia_nome), $eventos, $figuracao);
	...
	array_push($valores, $viajando, $dt_viagem_inicio, $dt_viagem_fim, toString($dt_validade_contrato), 2);	
	...
	if($_POST['avancar'] == "sim"){
		header("Location: /admin/elenco/caracteristicas_fisicas.php?id_elenco=$id_elenco");
	}
	else{
		header("Location: /admin/elenco/info_contato.php?id_elenco=$id_elenco");
	}
?>