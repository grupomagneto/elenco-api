...
function paginationCreator($maxPages,$currentPage){
	$html = '<div class="box_paginacao"><ul class="paginacao">';

	$html	.=	'<li><a href="'.$_SERVER['PHP_SELF'].'?pagina=1"><img src="img/setad.gif"></a></li>';

	$tmpNum = 1;
	while($tmpNum <= (int)$maxPages){
		if( $currentPage == $tmpNum ) {
			$html	.=	'<li>'.sprintf("%02d",$tmpNum).'</li>';
		}else{
			if( $tmpNum >= ($currentPage - 2) && $tmpNum <= ($currentPage + 2) ){
				$html	.=	'<li><a href="'.$_SERVER['PHP_SELF'].'?pagina='.$tmpNum.'">'.sprintf("%02d",$tmpNum).'</a></li>';
			}else if( $tmpNum <= 2 || $tmpNum >= ($maxPages - 1) ){
				$html	.=	'<li><a href="'.$_SERVER['PHP_SELF'].'?pagina='.$tmpNum.'">'.sprintf("%02d",$tmpNum).'</a></li>';
			}else if( $tmpNum == ($currentPage - 3) || $tmpNum == ($currentPage + 3) ){
				$html	.=	'<li>...</li>';
			}
		}
		$tmpNum++;
	...
	$html	.=	'<li><a href="'.$_SERVER['PHP_SELF'].'?pagina='.$maxPages.'"><img src="img/setadd.gif"></a></li>';
...
function getStatusAdministrador(){
	if(!$_SESSION['admlogado']){
		header("Location: /magnetoelenco/adm/adm_acesso.php");
	}
}
...
function enviaArquivo($arquivo, $nome_arquivo, $cd_elenco, $cd_tipo_foto, $diretorio, $data, $gera_thumb = false, $gera_pdf = false){
	...
	if(uploadArquivo($arquivo, $nome_formatado, $diretorio)){
		...
		if($gera_thumb){
			...
			file_get_contents("http://www.grupomagneto.com.br/magnetoelenco/v2/thumb.php?src=".$nome_formatado."&dest=".$nome_thumb."&x=72&y=72");
		}
		...
		if($gera_pdf){
			...
			file_get_contents("http://www.grupomagneto.com.br/magnetoelenco/v2/thumb_pdf.php?src=".$nome_formatado."&dest=".$nome_thumb214."&x=214&y=214");

			$nome_thumb103 = $array_arquivo[0]."_103x103.".$array_arquivo[1];
			file_get_contents("http://www.grupomagneto.com.br/magnetoelenco/v2/thumb_pdf.php?src=".$nome_formatado."&dest=".$nome_thumb103."&x=103&y=103");
		}
