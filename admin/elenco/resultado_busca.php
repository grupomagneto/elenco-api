...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td width="48%">
<?php
	// Recebe os parametros da pesquisa e define a condicao
	$pesquisa_condicao = intval($_REQUEST['condicao']);
	$pesquisa_nome     = limpaString($_REQUEST['nome_busca']);

	switch($pesquisa_condicao){
		case 1:
			$condicao_busca = "where publicado = 0";
			break;
		case 2:
			$condicao_busca = "where nu = 1";
			break;
		case 3:
			$condicao_busca = "where casting_corpo = 1";
			break;
		case 4:
			$condicao_busca = "where casting_maospes = 1";
			break;
		case 5:
			$condicao_busca = "where publicado = 1";
			break;
		default:
			$condicao_busca = "";
			break;
	...
	if($pesquisa_nome != ""){
		if($condicao_busca == ""){
			$condicao_busca = "where nome like '%$pesquisa_nome%' or nome_artistico like '%$pesquisa_nome%'";
		}
		else{
			$condicao_busca .= " and (nome like '%$pesquisa_nome%' or nome_artistico like '%$pesquisa_nome%')";
		}
	}

	include("../includes/form_busca_elenco.php");
?>
...
<?php
	...
	$sql_total = "select id_elenco from tb_elenco $condicao_busca";
...
<table width="98%" align="center" border="0" cellpadding="5" cellspacing="0">
  <tr bgcolor="#c3161c">
    <td class="font_br">
		<table>
			<tr>
				<td><a href="<? echo $_SERVER['PHP_SELF']."?ordenacao=nome&sentido=$link_sentido_nome&pagina=1&condicao=$pesquisa_condicao&nome_busca=$nome_busca"; ?>"><img src="/magnetoelenco/admin/img/<?= $img_seta_nome; ?>" border="0" /></a></td>
    ...
    <td class="font_br">
		<table>
			<tr>
				<td><a href="<? echo $_SERVER['PHP_SELF']."?ordenacao=nome_artistico&sentido=$link_sentido_artistico&pagina=1&condicao=$pesquisa_condicao&nome_busca=$nome_busca"; ?>"><img src="/magnetoelenco/admin/img/<?= $img_seta_artistico; ?>" border="0" /></a></td>
...
?>
	<h3 align="center">Nenhum registro encontrado.</h3>
<?php
	}
...
<table align="center" width="80%" border="0">
  <tr>
    ...
    <td width="50">
		...
		?>
		<a href="<? echo $_SERVER['PHP_SELF']."?ordenacao=$ordenacao&sentido=$sentido&pagina=1&condicao=$pesquisa_condicao&nome_busca=$nome_busca"; ?>">Primeira</a>
		<?php
		}
		?>
	</td>
    <td width="60"><?php
		if(!is_null($prev)){
		?>
			<a href="<? echo $_SERVER['PHP_SELF']."?ordenacao=$ordenacao&sentido=$sentido&pagina=$prev&condicao=$pesquisa_condicao&nome_busca=$nome_busca"; ?>"><< anterior</a>
		<?php
		}
		else{
			echo "<< anterior";
    ...
    <td align="center">
		...
		?>
			<a href="<? echo $_SERVER['PHP_SELF']."?ordenacao=$ordenacao&sentido=$sentido&pagina=$i&condicao=$pesquisa_condicao&nome_busca=$nome_busca"; ?>"><?= $print_i; ?></a>
		<?php
		}
		?>
	</td>
    <td width="60"><?php
		if(!is_null($next)){
		?>
			<a href="<? echo $_SERVER['PHP_SELF']."?ordenacao=$ordenacao&sentido=$sentido&pagina=$next&condicao=$pesquisa_condicao&nome_busca=$nome_busca"; ?>">pr&oacute;xima >></a>
		<?php
		}
		else{
			echo "pr&oacute;xima >>";
    ...
    <td width="50">
		...
		?>
		<a href="<? echo $_SERVER['PHP_SELF']."?ordenacao=$ordenacao&sentido=$sentido&pagina=$total_paginas&condicao=$pesquisa_condicao&nome_busca=$nome_busca"; ?>">Última</a>
		<?php
		}
		?>
	</td>
